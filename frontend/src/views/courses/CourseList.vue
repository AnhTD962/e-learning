<template>
  <div class="container mx-auto py-8">
    ...
    <div v-else-if="courseStore.error"
      class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative" role="alert">
    ...
    <div v-else-if="courseStore.courses.length === 0" class="text-center text-gray-500 mt-8">
      ...
      <p v-if="authStore.isTeacher() || authStore.isAdmin()" class="mt-4">
        <router-link to="/admin/courses" class="text-blue-600 hover:underline">Create a new course</router-link> to get
        started.
      </p>
    </div>
    <div v-if="authStore.isTeacher() || authStore.isAdmin()" class="text-right mb-4">
        <router-link to="/admin/courses"
          class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition duration-300 inline-block">
          Manage Courses
        </router-link>
      </div>
